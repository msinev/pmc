...
func readLoop(part io.Reader, fname string, code string) error {
...
func readLoopStub(part io.Reader) ([]byte, int64, error) {
	var read int64
	//var p float32
...
func uploadInitialFile(w http.ResponseWriter, r *http.Request) {
	if r.Method == "OPTIONS" {
	...
	for {
		...
		name := part.FormName()
		if name == "file" {
			err = readLoop(part, filePath, code)
			if err != io.EOF && err != nil {
